...
sub edit_be_block {
    ...
    return $app->permission_denied() if !$app->can_do('edit_custom_blocks');
    ...
    if (my $obj = MT->model('be_config')->load( { blog_id => $blog->id } )) {
        $param->{default_block_display_options} = $obj->block_display_options;
    }
...
sub can_save {
    ...
    return $author->permissions($blog_id)->can_do('edit_custom_blocks');
}
